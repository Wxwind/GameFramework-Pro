    ...
    public class NetworkChannelHelper : INetworkChannelHelper
    {
        ...
        public void Initialize(INetworkChannel networkChannel)
        {
            ...
            for (var i = 0; i < types.Length; i++)
            {
                ...
                {
                    var packetBase = (PacketBase)Activator.CreateInstance(types[i]);
                ...
                {
                    var packetHandler = (IPacketHandler)Activator.CreateInstance(types[i]);
        ...
        {
            var packetImpl = packet as PacketBase;
            if (packetImpl == null)
            {
        ...
        public Packet DeserializePacket(IPacketHeader packetHeader, Stream source, out object customErrorData)
        {
            ...
            if (scPacketHeader.IsValid)
            {
                ...
                if (packetType != null)
                    packet = (Packet)RuntimeTypeModel.Default.DeserializeWithLengthPrefix(source,
        ...
        {
            Type type = null;
            if (m_ServerToClientPacketTypes.TryGetValue(id, out type)) return type;
        ...
        private void OnNetworkConnected(object sender, GameEventArgs e)
        {
            ...
            if (ne.NetworkChannel != m_NetworkChannel) return;
