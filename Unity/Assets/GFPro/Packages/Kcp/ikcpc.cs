...
using static GFPro.IQUEUEHEAD;
using static GFPro.KCPBASIC;
...
namespace GFPro
{
    internal static unsafe class IKCP
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static int _iclamp_(int x, uint min, uint max)
        {
            return x < min ? (int)min : x > max ? (int)max : x;
        }
        ...
        private static void* ikcp_malloc(nint size)
        {
            return malloc(size);
        }
        ...
        {
            return iqueue_is_empty(&kcp->rcv_queue) ? -1 : ikcp_peeksize_internal(kcp);
        }
        ...
        {
            return kcp->nrcv_que < kcp->rcv_wnd ? (int)(kcp->rcv_wnd - kcp->nrcv_que) : 0;
        }
        ...
        public static void ikcp_streammode(IKCPCB* kcp, int stream)
        {
            kcp->stream = stream == 1 ? 1 : 0;
        }
