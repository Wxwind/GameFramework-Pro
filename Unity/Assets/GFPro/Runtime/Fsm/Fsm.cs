...
namespace GFPro
{
    ...
    internal sealed class Fsm<T> : FsmBase, IReference, IFsm<T> where T : class
    {
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            ...
            if (state == null)
            {
                throw new GameFrameworkException($"FSM '{new TypeNamePair(typeof(T), Name)}' can not start state '{typeof(TState).FullName}' which is not exist."
                );
            }
        ...
        public void Start(Type stateType)
        {
            ...
            if (state == null)
            {
                throw new GameFrameworkException($"FSM '{new TypeNamePair(typeof(T), Name)}' can not start state '{stateType.FullName}' which is not exist.");
            }
