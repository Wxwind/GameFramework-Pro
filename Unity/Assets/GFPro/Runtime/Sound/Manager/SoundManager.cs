...
namespace GFPro.Sound
{
    ...
    /// 声音管理器。
    /// </summary>
    internal sealed partial class SoundManager : ISoundManager
    {
        ...
        public void AddSoundAgentHelper(string soundGroupName, ISoundAgentHelper soundAgentHelper)
        {
            ...
            if (soundGroup == null)
            {
                throw new GameFrameworkException($"Sound group '{soundGroupName}' is not exist.");
            }
        ...
        private async UniTaskVoid InternalPlaySound(string soundAssetName, PlaySoundInfo playSoundInfo)
        {
            try
            {
                var audioClip = await m_ResourceComponent.LoadAssetAsync<UnityEngine.Object>(soundAssetName);
        ...
        public void PauseSound(int serialId, float fadeOutSeconds)
        {
            ...
            throw new GameFrameworkException($"Can not find sound '{serialId}'.");
        }
        ...
        private void LoadAssetSuccessCallback(string soundAssetName, UnityEngine.Object soundAsset,
            PlaySoundInfo playSoundInfo)
        {
            if (m_SoundsToReleaseOnLoad.Contains(playSoundInfo.SerialId))
            {
                ...
                ResourceComponent.Instance.UnloadAsset(soundAsset);
                return;
            }
            ...
            m_SoundsToReleaseOnLoad.Remove(playSoundInfo.SerialId);
            ResourceComponent.Instance.UnloadAsset(soundAsset);
            var errorMessage = $"Sound group '{playSoundInfo.SoundGroup.Name}' play sound '{soundAssetName}' failure.";
            if (playSoundInfo.PlaySoundParams.Referenced)
            {
                ReferencePool.Release(playSoundInfo.PlaySoundParams);
            }
            ...
            {
                Log.Info("sound failed to play because low priority");
            }
            ...
            PlaySoundInfo playSoundInfo)
        {
            ...
            m_SoundsBeingLoaded.Remove(playSoundInfo.SerialId);
            var appendErrorMessage = $"Load sound failure, asset name '{soundAssetName}', error message '{errorMessage}'.";
