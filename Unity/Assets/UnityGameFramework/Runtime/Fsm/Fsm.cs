...
namespace GFPro.Fsm
{
    ...
    internal sealed class Fsm<T> : FsmBase, IReference, IFsm<T> where T : class
    {
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            ...
            if (state == null)
            {
                throw new GameFrameworkException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name),
        ...
        internal void ChangeState(Type stateType)
        {
            ...
            if (state == null)
            {
                throw new GameFrameworkException(Utility.Text.Format("FSM '{0}' can not change state to '{1}' which is not exist.", new TypeNamePair(typeof(T), Name),
                    stateType.FullName));
            }
