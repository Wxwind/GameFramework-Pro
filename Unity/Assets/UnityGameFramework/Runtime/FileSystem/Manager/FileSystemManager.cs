...
namespace UnityGameFramework.FileSystem
{
    ...
    internal sealed class FileSystemManager : IFileSystemManager
    {
        ...
        public IFileSystem CreateFileSystem(string fullPath, FileSystemAccess access, int maxFileCount, int maxBlockCount)
        {
            ...
            var fileSystemStream = m_FileSystemHelper.CreateFileSystemStream(fullPath, access, true);
        ...
        public IFileSystem LoadFileSystem(string fullPath, FileSystemAccess access)
        {
            ...
            var fileSystemStream = m_FileSystemHelper.CreateFileSystemStream(fullPath, access, false);
            if (fileSystemStream == null)
            {
                throw new GameFrameworkException(Utility.Text.Format("Create file system stream for '{0}' failure.", fullPath));
            }
        ...
        {
            var index = 0;
            var results = new IFileSystem[m_FileSystems.Count];
            foreach (var fileSystem in m_FileSystems)
            {
