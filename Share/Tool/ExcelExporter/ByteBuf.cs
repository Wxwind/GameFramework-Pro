...
#pragma warning restore CA1819 // 属性不应返回数组
        ...
        public override string ToString()
        {
            ...
            for (int i = ReaderIndex; i < WriterIndex; i++) datas[i - ReaderIndex] = Bytes[i].ToString("X2");
        ...
        public static ByteBuf FromString(string value)
        {
            ...
            for (int i = 0; i < data.Length; i++) data[i] = byte.Parse(ss[i]);
        ...
        {
            int hash = 17;
            for (int i = ReaderIndex; i < WriterIndex; i++) hash = hash * 23 + Bytes[i];
            return hash;
        }
